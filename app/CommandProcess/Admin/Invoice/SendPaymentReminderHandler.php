...
class SendPaymentReminderHandler implements Handler
{
    ...
    public function handle(Command $command)
    {
        ...
        if (!$invoice->payment_reminder) {
            $this->commandBus->execute(new CreateInvoicePaymentReminder($command->invoiceId));
        }
        ...
        Mail::to($user->email)->send(new SendPaymentReminderMailToCustomer($invoice->user_name));
